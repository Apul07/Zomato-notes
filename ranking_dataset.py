"""
ranking_dataset.py
Part 2 sample dataset (verbatim). Titles are already in alphabetical
order, ready for the binary-search functions with no extra sorting step.
Seeded by seed.py as additional Note rows owned by owner_id: 1 with
tag: "kb-demo". The numeric `id` values below are illustrative labels
only — the database assigns its own autoincrement ids on seed.
"""
