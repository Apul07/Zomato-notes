"""
ai_sample_notes.py
Part 3 sample dataset (verbatim). Seeded by seed.py as additional Note
rows owned by owner_id: 2 with tag: "ai-demo". The numeric `id` values
below are illustrative labels only — the database assigns its own
autoincrement ids on seed; refer to notes by title afterwards.
"""
